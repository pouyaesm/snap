...
void node2vec(PWNet& InNet, const double& ParamP, const double& ParamQ,
  ...
  const bool& OutputWalks, TVVec<TInt, int64>& WalksVV,
	TIntFltVH& EmbeddingsHV, const int& typeCount, TFltV& typeWeight, const double& teleportP) {
...
// #pragma omp parallel for schedule(dynamic)
    for (int64 j = 0; j < NIdsV.Len(); j++) {
      ...
      TIntV WalkV;
	    SimulateWalk(InNet, NIdsV[j], WalkLen, Rnd, WalkV, typeCount, typeWeight, teleportP);
...
void node2vec(PWNet& InNet, const double& ParamP, const double& ParamQ,
	const int& Dimensions, const int& WalkLen, const int& NumWalks,
	const int& WinSize, const int& Iter, const bool& Verbose, const bool& OutputWalks,
	TVVec<TInt,
	int64>& WalksVV,
	TIntFltVH& EmbeddingsHV){
		TFltV typeWeight;
		typeWeight.Add(1);
		node2vec(InNet, ParamP, ParamQ, Dimensions,
			WalkLen, NumWalks, WinSize,
			Iter, Verbose, OutputWalks, WalksVV, EmbeddingsHV, 1, typeWeight, 0.0);
}
